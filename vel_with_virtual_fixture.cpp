		...
		if(initialized_phantom)
		{
			...
			else
			{
				...
				else
				{
					...
					iiwa_prev_in_camera = transform2(tf::Vector3(prev_x_iiwa_position,prev_y_iiwa_position,prev_z_iiwa_position));
					iiwa_init_pos_in_camera = transform2(tf::Vector3(xyz_iiwa_init.linear.x,xyz_iiwa_init.linear.y,xyz_iiwa_init.linear.z));
					...
					ROS_INFO_STREAM(iiwa_prev_in_camera.z()<<"  "<<ref.linear.z);
					if(iiwa_prev_in_camera.z()<ref.linear.z)
					{
						ref.linear.x=iiwa_prev_in_camera.x();
						ref.linear.y=iiwa_prev_in_camera.y();
						ref.linear.z=iiwa_prev_in_camera.z(); //xyz_phantom.linear.z*z_gain
